  ...
  return (
    <div className="bg-[#fcfcfc] min-h-screen font-sans">
      ...
      <header className="max-w-6xl mx-auto px-6 pt-24 pb-12 text-center">
        ...
        <p className="text-xl text-gray-400 font-medium mb-12">The world's smartest AI directory, curated by Mantu Patra.</p>
        ...
        <div className="max-w-2xl mx-auto mb-12 relative group">
          ...
          />
          <div className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-300 font-black">SEARCH</div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredTools.map((tool) => (
            <Link href={`/tool/${tool.slug}`} key={tool.id} className="group">
              <div className="bg-white p-8 rounded-[2rem] border border-gray-50 hover:border-blue-600 transition-all hover:shadow-2xl hover:shadow-blue-100 h-full flex flex-col">
                ...
                <div className="mt-auto font-black text-[10px] uppercase tracking-[0.2em] text-gray-300 group-hover:text-blue-600">View Full Review →</div>
              </div>
            </Link>
          ))}
        </div>
