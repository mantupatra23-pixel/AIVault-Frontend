...
type DecisionResponse = {
  requirements?: DecisionRequirements | null;
...
export default function AIFinderPage() {
  ...
  const [requirements, setRequirements] = useState<DecisionRequirements | null>(null);
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-950">
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/compare"
              className="rounded-xl border border-slate-200 bg-white px-3.5 py-1.5 text-xs font-bold text-slate-700 transition hover:border-blue-300 hover:text-blue-600"
            >
              ⚖️ Compare
            </Link>
            <Link
      ...
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-5xl px-4 py-12 text-center sm:px-6 sm:py-16">
          ...
          <h1 className="text-3xl font-black tracking-tight sm:text-5xl">
            ...
            <br />
            <span className="text-blue-600">for your exact needs.</span>
          </h1>

          <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 text-slate-500 sm:text-base">
            Tell AI Vault what you want to accomplish. Our decision engine
            compares the available AI tools and ranks the best matches.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        >
          <div className="grid gap-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              ...
              >
                {loading ? "Finding best tools..." : "Find Best AI Tools →"}
              </button>
        ...
        {results.length > 0 && (
          <section className="mt-8">
            <div className="mb-5">
              ...
              <h2 className="mt-1 text-2xl font-black">Best matching tools</h2>
              <p className="mt-2 text-sm text-slate-500">
                Ranked using your requirements, available tool data, and deterministic matching rules.
              </p>
            </div>

            <div className="space-y-4">
              {results.map((item, index) => {
                const name = item.tool?.name || "Unnamed tool";
                const slug = item.tool?.slug;
                const score = typeof item.match_score === "number" ? item.match_score : 0;
        ...
        {/* NO RESULTS */}
        {!loading && requirements && results.length === 0 && !error && (
          <section className="mt-8 rounded-3xl border border-slate-200 bg-white p-8 text-center">
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-100 text-2xl">
              🔎
            </div>
            <h2 className="mt-4 text-xl font-black">No strong matches found</h2>
            <p className="mx-auto mt-2 max-w-lg text-sm leading-6 text-slate-500">
              Try a broader category, different platform, or a less restrictive budget.
            </p>
          </section>
        )}
        ...
        {!requirements && !loading && (
          <section className="mt-8 grid gap-4 sm:grid-cols-3">
            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              ...
              <h3 className="mt-3 font-bold">Goal Matching</h3>
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Match tools against what you actually want to accomplish.
              </p>
            </div>

            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              <div className="text-2xl">⚡</div>
              <h3 className="mt-3 font-bold">Smart Ranking</h3>
              <p className="mt-2 text-sm leading-6 text-slate-500">
                See match scores and confidence instead of a generic tool list.
              </p>
            </div>

            <div className="rounded-2xl border border-slate-200 bg-white p-5">
              <div className="text-2xl">🧠</div>
              <h3 className="mt-3 font-bold">Explainable Results</h3>
              <p className="mt-2 text-sm leading-6 text-slate-500">
                Every recommendation explains why the tool matched your requirements.
              </p>
            </div>
          </section>
        )}
