...
export default async function ToolPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!tool) return <div className="p-20 text-center font-black uppercase">Tool Not Found</div>;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,380px] gap-20">
          <div className="space-y-20">
            {sections.map((section: string, idx: number) => {
              ...
              if (["PROS", "CONS", "FAQ", "QUESTIONS"].some(k => rawTitle.toUpperCase().includes(k)) && !isIntro) return null;
            ...
            {sections.map((section: string, idx: number) => {
              ...
              return (
                <div key={idx} className={`p-10 rounded-[3rem] border transition-all hover:shadow-xl ${
                  ...
                  <div className="text-lg leading-relaxed whitespace-pre-wrap font-semibold">{cleanedBody}</div>
                </div>
              );
            })}
          </aside>
