    ...
    <main className="min-h-screen bg-white text-slate-900 font-sans pb-20 selection:bg-blue-600 selection:text-white">
      ...
      <nav className="fixed top-0 left-0 right-0 h-20 bg-white/80 backdrop-blur-xl z-[100] border-b border-gray-100 flex items-center justify-between px-6 md:px-12">
        ...
        <div className="flex items-center gap-4">
          <a href={finalUrl} target="_blank" rel="noopener noreferrer" className="bg-black text-white text-[10px] px-8 py-3 rounded-full font-black uppercase tracking-widest hover:bg-blue-600 transition-all">
            Visit Portal
          </a>
        </div>
      </nav>
      ...
      <article className="max-w-4xl mx-auto px-6 pt-44">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-32">
          <div className="bg-[#f0fff4] p-12 rounded-[50px] border border-green-100 shadow-sm">
            ...
            <div className="text-lg font-bold text-green-900 whitespace-pre-line italic">
          ...
          <div className="bg-[#fff5f5] p-12 rounded-[50px] border border-red-100 shadow-sm">
            ...
            <div className="text-lg font-bold text-red-900 whitespace-pre-line italic">
              {tool.cons || "• API Credit Dependent\n• Learning Curve"}
            </div>
          </div>
        </div>

        {/* 💰 PRICING CTA (CLICKABLE FIX) */}
        <section className="relative z-20 bg-white border-[8px] border-black rounded-[60px] p-12 md:p-24 text-center shadow-2xl mb-40">
          ...
          {/* 🔥 LINK FIXED WITH TARGET BLANK */}
          <a 
            href={finalUrl} 
            target="_blank" 
            rel="noopener noreferrer"
            className="relative z-30 inline-block bg-blue-600 text-white px-16 py-7 rounded-full font-[1000] text-2xl uppercase tracking-widest hover:scale-105 active:scale-95 transition-all shadow-2xl shadow-blue-600/40 cursor-pointer"
          >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {related?.map((t) => (
              <Link 
                key={t.slug} 
                href={`/tool/${t.slug}`} 
                className="group p-8 bg-white border border-gray-100 rounded-[40px] hover:border-blue-600 transition-all hover:-translate-y-2 duration-500 block relative z-10"
              >
